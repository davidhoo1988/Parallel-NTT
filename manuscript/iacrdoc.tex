...
\KwIn{bootstrapping key brK $\mathbf{Z}_{i,j,v}=RGSW(X^{vB_r^j\cdot s_i})$ for all $i, j, v$,  RLWE ciphertext $ct=RLWE(X^b)$ and a public known vector $\mathbf{a}$.}
\KwOut{an RLWE encryption of $X^{b-\langle \mathbf{a}, \mathbf{s}\rangle}$.}
   \For{$i\leftarrow 0$ \KwTo $n-1$}{
        Compute $a_i\gets q-a_i$\;
        \For{$j\leftarrow 0$ \KwTo $log_{B_r}q-1$}{
            Compute $a_{i,j}\gets \lfloor a_i/B_r^j\rfloor$\;
            Compute $ACC\gets ACC\odot \mathbf{Z}_{i,j,v}$\;
        }
   }
  \Return {$ACC$}
...
\begin{algorithm}[!tbh]
\DontPrintSemicolon % Some LaTeX compilers require you to use \dontprintsemicolon instead
\KwIn{ bootstrapping key brK,  LWE ciphertexts $ct_i=LWE_{\mathbf{s}}^{q/4}(m_i)=(\mathbf{a},b) \text{ for } i\in \{0,1\}$, LWE-to-LWE key-switching key ksK.}
\KwOut{ an LWE encryption of $\texttt{NAND}(m_0,m_1)$, \textit{i.e.,}  $ct_q=LWE_{\mathbf{s}}^{q/4}(\texttt{NAND}(m_0,m_1))$. }
...
\begin{equation}\label{eq:ntt_def}
